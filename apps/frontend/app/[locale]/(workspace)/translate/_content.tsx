...
export default function TranslatePageContent() {
  ...
  const commitLetter = useCallback((letter: string, confidence: number) => {
    ...
    if (voiceEnabledRef.current && "speechSynthesis" in window) {
      ...
      u.lang = localeConfig.ttsLang;
      u.rate = 0.95;
      u.onend = () => setIsSpeaking(false);
      u.onerror = () => setIsSpeaking(false);
      window.speechSynthesis.speak(u);
    }
  }, [localeConfig.ttsLang, t]);
  ...
  const handleSpeak = useCallback(() => {
    ...
    window.speechSynthesis.speak(u);
  }, [tokens, isSpeaking, prefs.ttsSpeed, prefs.ttsVolume, localeConfig.ttsLang]);
  ...
  const handleSpeakEntry = useCallback(
    ...
    [prefs.ttsSpeed, prefs.ttsVolume, localeConfig.ttsLang]
  );
  ...
  const TABS = [
    { key: "hasil" as MobileTab, label: t("resultTab"), icon: Terminal },
    { key: "kalimat" as MobileTab, label: t("sentenceTab"), icon: Layers },
    { key: "riwayat" as MobileTab, label: t("historyTab"), icon: Activity },
  ] as const;
