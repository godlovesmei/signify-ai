        ...
        <div className="grid min-h-full gap-5 p-4 md:p-6 lg:grid-cols-[280px_minmax(0,1fr)_320px] lg:p-8">
          <aside className="flex min-w-0 flex-col gap-5">
            <section className="rounded-sm border border-cohere-hairline bg-cohere-stone p-5">
              <p className="mb-5 text-mono-label text-[12px] text-cohere-slate">{t("target")}</p>
            ...
            <section className="rounded-sm border border-cohere-hairline bg-cohere-canvas p-5">
              <div className="flex items-center justify-between gap-3">
                <p className="text-mono-label text-[12px] text-cohere-slate">{t("performance")}</p>
                {isStatsLoading ? (
                  <Loader2 className="size-4 animate-spin text-cohere-slate" aria-label={t("loading")} />
                ) : statsLoadError ? (
                  <button
                    type="button"
              ...
              <div className="mt-5 grid grid-cols-3 gap-3 lg:grid-cols-1">
                {[
                  [t("samples"), stats.totalAttempts],
                  [t("precision"), `${stats.totalAttempts === 0 ? 0 : Math.round((stats.correctAttempts / stats.totalAttempts) * 100)}%`],
              ...
              {weakLetters.length > 0 && (
                <div className="mt-6 border-t border-cohere-hairline pt-5">
                  <p className="mb-3 text-mono-label text-[11px] text-cohere-slate">{t("focusQueue")}</p>
          ...
          <aside className="flex min-w-0 flex-col gap-5">
            <section className="rounded-sm border border-cohere-hairline bg-cohere-canvas p-5">
              <p className="text-mono-label text-[12px] text-cohere-slate">{t("holdProgress")}</p>
              <div className="mt-6 flex flex-col items-center gap-6">
                ...
                <div className="w-full">
                  ...
                  <p className="mt-3 text-center text-[12px] text-cohere-slate">{t("maintainGesture")}</p>
                </div>
              </div>
            </section>

            <section className="rounded-sm border border-cohere-hairline bg-cohere-stone p-5">
              <p className="mb-5 text-mono-label text-[12px] text-cohere-slate">{t("controls")}</p>
              <div className="flex flex-col gap-3">
                ...
                >
                  {t("skipTarget")} <ChevronRight className="size-4" />
                </Button>

                <Dialog>
                  <DialogTrigger asChild>
                    <Button variant="outline" className="justify-between">
                      {t("sessionConfigShort")} <Sliders className="size-4" />
                    </Button>
                  </DialogTrigger>
                  <DialogContent className="max-w-[420px] rounded-lg border-cohere-hairline bg-cohere-canvas p-6 text-cohere-ink">
                    <DialogHeader>
                      <DialogTitle className="text-[24px] leading-[1.3]">{t("sessionConfig")}</DialogTitle>
                      <DialogDescription className="sr-only">
                        {t("sessionConfigDescription")}
                      </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between border-t border-cohere-hairline pt-4">
                        <span className="text-[14px] text-cohere-ink">{t("guideOverlay")}</span>
                        <Switch checked={ghostVisible} onCheckedChange={setGhostVisible} />
                      </div>
                      <div className="flex items-center justify-between border-t border-cohere-hairline pt-4">
                        <span className="text-[14px] text-cohere-ink">{t("insightMetrics")}</span>
                ...
                >
                  {ghostVisible ? t("hideGuide") : t("showGuide")}
                </button>
              </div>
            </section>
